...
from svd import train, train_sentences, test, test_sentences, unique_words
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix

class NewsDataset(Dataset):
    def __init__(self, texts, labels, word_to_idx):
        self.texts = [torch.tensor([word_to_idx["<unk>"] if word not in word_to_idx or word_to_idx[word] < 4 else word_to_idx[word] for word in text], dtype=torch.long) for text in texts]
...
embedding_matrix_svd = torch.load('svd-word-vectors.pt')
model = RNNClassifier(embedding_matrix_svd, hidden_dim=24, output_dim=4)
